 ...
 *
 * Última mofidicación: 10/10/2025.
 *
 * @author Robert Sallent
 * 
 * @since v1.7.4 añadido el método getFiles() y el método estático files().
 * @since v2.1.3 añadidos el método getFileNames() y el método estático fileNames()
 *
 */
...
class FileList{
    ...
    /**
     * Recupera la lista de nombres de fichero en el directorio de trabajo.
     * Recupera un array de strings (nombres de fichero).
     * Para filtrar, puede recibir una expresión regular o un listado de extensiones
     *
     * @param bool $extension true para nombres de fichero con extensión o false para solamente el nombre de fichero sin extensión
     * @param string|array $matches expresión regular o lista de extensiones
     * @param bool $specialEntries permite indicar si queremos que aparezcan el . y el ..
     *
     * @return array listado de entradas de directorio coincidentes con los filtros aplicados, ordenadas alfabéticamente
    ...
    public function getFileNames(
        ...
        $paths = $this->getEntries($matches, $specialEntries);
        
        // prepara la lista para los nombres
        $fileNames = [];
        
        // guarda los nombres en la lista (con o sin extensión)
        foreach($paths as $path)
            $fileNames[] =  pathinfo($path, $extension ? PATHINFO_FILENAME : PATHINFO_FILENAME);
    ...
    /**
     * Método estático que recupera la lista de nombres de fichero en el directorio de trabajo.
     * 
     * Recupera un array de strings (nombres de fichero).
     * Para filtrar, puede recibir una expresión regular o un listado de extensiones
     *
     * @param bool $extensions true para nombres de fichero con extensión o false para solamente el nombre de fichero sin extensión
     * @param string $directorio directorio de trabajo
     * @param string|array $matches expresión regular o lista de extensiones
     * @param bool $specialEntries permite indicar si queremos que aparezcan el . y el ..
     *
    ...
    public static function fileNames(
        ...
        return (new FileList($directorio))->getFileNames($extensions, $matches, $specialEntries);
    }
}